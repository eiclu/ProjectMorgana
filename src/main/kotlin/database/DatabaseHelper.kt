...
class DatabaseHelper() {
    ...
    @Language("MySQL")
    fun getUserById(id: Long): User? = getDatabaseResult("SELECT UserId, CurrentSemester, Major, IsAdmin FROM Users WHERE UserId=$id", User)?.firstOrNull()
    ...
    fun getCoursesForUser(userId: Long): Set<Course> {
        ...
        """.trimIndent()
        return getDatabaseResult(getCoursesQuery, Course) ?: emptySet()
    }
    ...
    fun updateSubscribedCourses(userId: Long, enabledCourses: Collection<Long>) {
        ...
        //language=MySQL
        updateDatabase("INSERT INTO User_Course_Links (UserId, CourseId) VALUES $enabledCoursesQuery")
    }
    ...
    fun addCourse(subject: String, module: String?, course: String, shorthand: String?) {
        //language=MySQL
        updateDatabase("INSERT INTO Courses (Subject, Module, Course, ShortHand) VALUES (${typeOrNull(subject)}, ${typeOrNull(module)}, ${typeOrNull(course)}, ${typeOrNull(shorthand)})")
    }
    ...
    fun getCourseById(id: Long): Course? {
        ...
        val getCoursesQuery = """
            SELECT TempCourses.CourseId, Subject, Module, Course, Shorthand, UserCount FROM TempCourses
            WHERE TempCourses.CourseId=$id
        """.trimIndent()
        return getDatabaseResult(getCoursesQuery, Course)?.firstOrNull()
    }

    @Language("MySQL")
    fun getChannelsForCourse(id: Long): Set<Channel> = getDatabaseResult("SELECT ChannelId FROM Channels WHERE CourseId=$id", Channel) ?: emptySet()
    ...
    fun addUser(user: User) {
        //language=MySQL
        updateDatabase("INSERT INTO Users (UserId, CurrentSemester, Major) VALUES (${typeOrNull(user.userId)}, ${typeOrNull(user.currentSemester)}, ${typeOrNull(user.mayor)})")
    }
    ...
    fun getUsersForCourse(id: Long): Set<User> = getDatabaseResult("""
        ...
        WHERE UCL.CourseId=$id
    """.trimIndent(), User) ?: emptySet()
    ...
    fun updateCourseInfo(id: Long, map: Map<String, String?>) {
        ...
        val update = updateMap.map { "${it.key} = ${typeOrNull(it.value)}" }.joinToString(", ")
        //language=MySQL
    ...
    private fun <T : Any> getDatabaseResult(query: String, factory: DatabaseItemInterface<T>): Set<T>? = getOrNull(log) {
        log.trace(query)
        val dbResult = dbConnection.createStatement().executeQuery(query)
        generateSequence { if (dbResult.next()) factory.generateItem(dbResult) else null }.toSet()
    }

    private fun hasResult(query: String): Boolean = getOrNull(log) { dbConnection.createStatement().executeQuery(query).next() } ?: false

    private fun updateDatabase(query: String): Boolean {
